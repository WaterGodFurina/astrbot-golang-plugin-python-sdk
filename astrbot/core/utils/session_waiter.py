...
**与本体/Go 宿主的差异（降级说明）**：
本体中 `SessionWaiter.trigger()` 由 star_manager 在处理每条入站消息时
根据 FILTERS 匹配后自动调用（见 builtin_stars/astrbot/main.py 的
handle_session_control_agent）。Go 宿主没有把“每一条新消息”注入 Python
侧的机制，因此：

- `register_wait()` 会正常挂起等待（不抛 NotImplementedError）；
- 等待期间若无任何来源调用 `SessionWaiter.trigger()`，则按超时时间
  （默认 30 秒）抛 `TimeoutError` 结束会话——插件普遍捕获该异常并
  发送“没有收到回复”等提示，逻辑不崩溃；
- 需要手动喂入下一条消息时（例如宿主侧后续接入、或插件自行接管
  消息分发），调用 `await SessionWaiter.trigger(session_id, event)`
  即可，行为与本体完全一致。
...
class SessionWaiter:
    ...
    async def register_wait(
        ...
        注（Go 宿主降级）：宿主不会自动触发等待的会话，register_wait 将
        挂起至超时（抛 TimeoutError，与本体行为一致），或由宿主/插件
        主动调用 `await SessionWaiter.trigger(session_id, event)` 喂入消息。
        """
