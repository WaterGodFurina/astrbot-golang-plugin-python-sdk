...
对齐 Python 本体 `astrbot.core.utils.error_redaction`：`safe_error(prefix, exc)`
把异常/错误信息中的 API Key、Token、Authorization、sk- 密钥等敏感内容
替换为 [REDACTED]，避免写入日志/发给用户时泄露凭据。
"""
import re
...
def redact_sensitive_text(text: str) -> str:
    """对文本中的敏感信息（API Key/Token/密钥）做脱敏处理。"""
...
def safe_error(
    ...
    *,
    ...
    Args:
        prefix: 错误信息前缀（调用方上下文，如 "获取模型列表失败: "）。
        error: 异常对象或错误字符串。
        redact: 是否对敏感信息脱敏（默认 True）。
