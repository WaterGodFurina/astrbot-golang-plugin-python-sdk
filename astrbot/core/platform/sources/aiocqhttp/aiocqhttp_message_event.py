"""OneBot v11 (aiocqhttp) 消息事件 —— 移植自 Python AstrBot。

事件对象的 bot 为 aiocqhttp 兼容层实例（宿主桥），send/get_group 等经
HostBridge.call_action 转发到宿主 Go 的 aiocqhttp 适配器。
"""
import asyncio
import re
...
class AiocqhttpMessageEvent(AstrMessageEvent):
    ...
    async def send(self, message: MessageChain) -> None:
        """发送消息"""
        # 对齐 Python 本体：发送操作标记（阻止管线继续走 LLM）。
        ...
        # 嵌入式宿主：send 已实际发送（经 bot → 宿主），不再调 super().send
        # 避免二次发送。
